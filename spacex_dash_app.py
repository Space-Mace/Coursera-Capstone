...
def get_pie_chart(entered_site):
    ...
    if entered_site == 'ALL':
        ...
        new_df = filtered_df[filtered_df['Launch Site'] == entered_site]
        new_df = new_df[['Launch Site', 'class']].groupby('class').size().reset_index(name='class count')
        fig = px.pie(new_df,
        values='class count',
        names='class',
...
def get_scatter_plot(entered_site, payload_slider):
    ...
    if entered_site == 'ALL':
        ...
        new_df2 = filtered_df2[filtered_df2['Launch Site'] == entered_site]
        fig=px.scatter(new_df2, 
        x='Payload Mass (kg)', 
        y='class', 
        color='Booster Version Category',
        title='Correlation Between Payload and Launch Success')
